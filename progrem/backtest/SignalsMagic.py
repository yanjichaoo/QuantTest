...
# =====简单布林策略
# 策略
def signal_simple_bolling(df, para=[200]):
    """
    :param df:
    :param para: n, m
    :return:
    ...
    df['mtm_c2'] = abs(df['mtm_high'] - df['mtm_close'].shift(1))
    df['mtm_c3'] = abs(df['mtm_low'] - df['mtm_close'].shift(1))
    ...
    df['signal'] = df[['signal_short', 'signal_long']].sum(axis=1)
...
# 策略参数组合
def signal_simple_bolling_para_list(m_list=range(10, 1000, 5)):
    """
    产生布林 策略的参数范围
    :param m_list:
    :param n_list:
